...
mod tests {
    ...
    fn test_bitcoin_script_p2ms() {
        ...
        let bytes = [
            0x52_u8, 0x21, 0x02, 0x2d, 0xf8, 0x75, 0x04, 0x80, 0xad, 0x5b, 0x26, 0x95, 0x0b, 0x25,
    ...
    fn test_bitcoin_script_p2sh() {
        ...
        let bytes = [
            0xa9_u8, 0x14, // OP_HASH160, OP_PUSHDATA0(20 bytes)
    ...
    fn test_bitcoin_script_non_standard() {
        ...
        let bytes = [0x73_u8, 0x63, 0x72, 0x69, 0x70, 0x74];
        let result = evaluate_script(Script::from_bytes(&bytes), Network::Bitcoin);
        assert_eq!(result.addresses.first(), None);
        assert_eq!(result.pattern, ScriptType::NotRecognised);
    }

    #[test]
    fn test_bitcoin_bogus_script() {
        let bytes = [0x4c_u8, 0xFF, 0x00];
        let result = evaluate_script(Script::from_bytes(&bytes), Network::Bitcoin);
        assert_eq!(result.addresses.first(), None);
        assert_eq!(result.pattern, ScriptType::NotRecognised);
    }
